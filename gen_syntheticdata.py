...
ds_code_test = Dataset.load_from_disk("/home/sureshm/ssuresh/code_porting_models/dataset/babeltower")
#ds_code_test = ds_code_test.select(range(10))
...
if infer:
    template = """
                ...
                {{[code analysis]: <detailed analysis of the given code for optimization and parallelization here> \n}}
                {{[actions to optimize, parallelize, and transform the code]: <detailed actions sequence to optimize, parallelize, and transform the code here>\n}}
                {{[Transformed CUDA code]: <Final transformed code here> \n}}
                {{[CUDA kernel code]: <Just the kernel from the transformed code here> \n}}
                Always provide the complete translated CUDA code, do not skip any part of the code. \n
                In the code section, always only provide the code without any additional text or explanation. \n
                Use the same variable names as in the input code. \n
                Do not use any 'Markdown' formatting for text \n
                Start the output with '<think>\n' \n
                """

    def append_to_json(file_path, new_data: dict):
    ...
    def parsemessage(ai_message: AIMessage) -> str:
        """Parse the AI message."""
        message = ai_message.content.lower()
        message = str(message)
        return message
    ...
    references = []
    ############## Inference all ##############
    for idx, cpp_code in tqdm(enumerate(ds_code_test["code"])):
        ...
        res = parsemessage(AIMessage(content=response.content))
        res_dict = constructOPjson(idx=idx, model_name=model_name, cpp_code=cpp_code, res=res)
        output_file = model_name + "Syntheticds_qwen3.json"
